...
#define LT_JSFN LT(JSFN,KC_SPC)
#define LT_USFN LT(USFN,KC_SPC)

#define CM_JRISE  LT(JRISE,KC_ESC)
#define CM_URISE  LT(URISE,KC_ESC)

#define LT_JLSYM  LT(JRISE,KC_K)
#define LT_JRSYM  LT(JRISE,KC_D)
#define LT_ULSYM  LT(URISE,KC_K)
#define LT_URSYM  LT(URISE,KC_D)
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    //win jpkey(QMK)  GUI(command)   CTL(control)   ALT(option)
        ...
        A_ALT,   KC_S,    KC_D,     KC_F,    G_GUI,         KC_H,    KC_J,    KC_K,     KC_L,    CM_FNSC,
    ...
    [JLEF] = LAYOUT_split_3x5_3(
        CM_ALCT,  CM_ALT4,  LT_EXCL,  SW_ATAB,  CM_WSCS,        CM_K_QY,  CM_K_QU,  CM_K_QI,  CM_K_QO,  CM_K_QP,
    ...
    [EXCL] = LAYOUT_split_3x5_3(
        _______,  CM_CHOM,  _______,  CM_CEND,  _______,        _______,  CM_CHOM,  KC_PGUP,  CM_CEND,  _______,
        _______,  CM_CPGU,  _______,  CM_CPGD,  _______,        _______,  CM_CPGU,  KC_PGDN,  CM_CPGD,  _______,
        _______,  _______,  _______,  _______,  _______,        _______,  _______,  _______,  _______,  _______,
                            _______,  _______,  _______,        _______,  _______,  _______
    ),
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  ...
  switch (keycode) {
    ...
    case CM_SPFN:
      if (get_highest_layer(default_layer_state)==JBASE){
        user_lt(record,JSFN,KC_SPC,&spfn_pressed,&spfn_pressed_time,false,&mod_switch_keycode);
      } else {
        user_lt(record,USFN,KC_SPC,&spfn_pressed,&spfn_pressed_time,false,&mod_switch_keycode);
      }
      return false;
      break;
    case CM_FNSC:
