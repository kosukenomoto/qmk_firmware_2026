...
#define LN2_SFT SFT_T(KC_LANG2)//KC_MHEN
#define LN1_CTL LCTL_T(KC_LANG1)
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    //win jpkey(QMK)  GUI(command)   CTL(control)   ALT(option)
        ...
        Z_SFT,   KC_X,    KC_C,     KC_V,    KC_B,          KC_N,    KC_M,    KC_COMM,  KC_DOT,  SL_SFT,
                          CM_JQFN,  LN1_CTL, CM_LISE,       CM_SPFN, LN2_SFT, CM_SYS
    ),   
    ...
    [JQFN] = LAYOUT_split_3x5_3( /* [> QFN<] */
        _______,  _______,  _______,  _______,  _______,        _______,  KC_HOME,  KC_UP,    KC_END,   KC_MINS,
        CM_GUIM,  CM_WSCS,  KC_DEL,   CM_GUIT,  _______,        KC_BSPC,  KC_LEFT,  KC_DOWN,  KC_RGHT,  KC_ENT,
        CM_GUIE,  CM_ALT4,  KC_LGUI,  _______,  _______,        _______,  _______,  _______,  _______,  _______,
                            _______,  _______,  _______,        _______,  _______,  _______   
    ),
...
//    }
  ...
  switch (keycode) {
    ...
    case CM_SPFN2:
      if (get_highest_layer(default_layer_state)==JBASE){
        user_lt(record,JSPFN,KC_LGUI,&spfn_pressed,&spfn_pressed_time,false);
      } else {
        user_lt(record,USPFN,KC_LGUI,&spfn_pressed,&spfn_pressed_time,false);
      }
      return false;
      break;
    case CM_FNSC:
